...
export default function Scene() {
  ...
  const [status, setStatus] = useState('Connecting to the universe…')
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      setPlace(nearest ? system.names.get(nearest.id) ?? '' : 'Between the stars')
  ...
  return <>
    ...
    <div style={{ position: 'fixed', top: 18, left: 22, color: '#e8ecdc', pointerEvents: 'none', font: '13px/1.7 system-ui', textShadow: '0 2px 5px #000' }}>
      ...
      <div>{status}</div>
      <div style={{ opacity: 0.75 }}>WASD — explore · Space — jump · C — call ship · E — board / land</div>
      {perf && <pre>{perf}</pre>}
    </div>
