...
export class TerrainChunk {
  ...
  sampleVisualRadius(dirLike: Vec3Like): number | null {
    ...
    const gx = tx * (GRID_SIZE - 1)
    const gy = ty * (GRID_SIZE - 1)
    const ix = Math.min(GRID_SIZE - 2, Math.max(0, Math.floor(gx)))
    const iy = Math.min(GRID_SIZE - 2, Math.max(0, Math.floor(gy)))

    let best: number | null = null
    for (let y = Math.max(0, iy - 1); y <= Math.min(GRID_SIZE - 2, iy + 1); y++) {
      for (let x = Math.max(0, ix - 1); x <= Math.min(GRID_SIZE - 2, ix + 1); x++) {
